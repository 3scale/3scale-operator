...
func PortaClientFromURLString(adminURLStr string) (*client.ThreeScaleClient, error) {
	adminURL, err := url.Parse(adminURLStr)
	if err != nil {
		return nil, err
	}
	return PortaClient(adminURL)
}

// PortaClient instantiates porta_client.ThreeScaleClient from admin url object
func PortaClient(url *url.URL) (*client.ThreeScaleClient, error) {
	...
	return client.NewThreeScale(adminPortal, &http.Client{Transport: tr}), nil
}
