	...
	testCases := []struct {
	...
	}{
		{
			...
			testBody: func(t *testing.T, r *reconcilers.BaseReconciler, req reconcile.Request) {
				...
				require.Error(t, err)
				require.Equal(t, err.Error(), "developeraccounts.capabilities.3scale.net \"test\" not found")
		...
		},
		{
			...
			testBody: func(t *testing.T, r *reconcilers.BaseReconciler, req reconcile.Request) {
				...
				condition := currentApplication.Status.Conditions.GetCondition(capabilitiesv1beta1.ApplicationReadyConditionType)
				require.Equal(t, corev1.ConditionFalse, condition.Status)
				require.Equal(t, "LookupProviderAccount: no provider account found", condition.Message)
			},
		},
		{
			...
			testBody: func(t *testing.T, r *reconcilers.BaseReconciler, req reconcile.Request) {
				...
				condition := currentApplication.Status.Conditions.GetCondition(capabilitiesv1beta1.ApplicationReadyConditionType)
				require.Equal(t, corev1.ConditionFalse, condition.Status)
				require.Equal(t, `reconcile3scaleApplication application [test]: plan [unknown] doesnt exist in product [test]`, condition.Message)
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.httpHandler != nil {
				httpServer = httptest.NewServer(tc.httpHandler)
