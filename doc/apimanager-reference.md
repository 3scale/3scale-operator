...
| SentinelIsUsed              | `sentinelIsUsed` | `bool` | No | `nil` | Allow Sentinel ACL credentials to be set via environment variables in pods. When enabled (set to true) and external components are configured, the Redis ACL *_SENTINEL_* credentials are injected into system or/and backend pods                                                                                                                                                                                                                                                                                                                                                          |
...
| CONFIG_REDIS_USERNAME | Backend's redis ACL creds - username. | `""` |
| CONFIG_REDIS_PASSWORD | Backend's redis ACL creds - password. | `""` |
| CONFIG_REDIS_SENTINEL_USERNAME | Backend's redis sentinel ACL creds - username. Used when `spec.sentinelIsUsed: true` in `ApiManager CR` | `""` |
| CONFIG_REDIS_SENTINEL_PASSWORD | Backend's redis sentinel ACL creds - password. Used when `spec.sentinelIsUsed: true` in `ApiManager CR` | `""` |
| CONFIG_QUEUES_USERNAME | Backend's redis queues ACL creds - username. | `""` |
| CONFIG_QUEUES_PASSWORD | Backend's redis queues ACL creds - password. | `""` |
| CONFIG_QUEUES_SENTINEL_USERNAME | Backend's redis queues sentinel ACL creds - username. Used when `spec.sentinelIsUsed: true` in `ApiManager CR` | `""` |
| CONFIG_QUEUES_SENTINEL_PASSWORD | Backend's redis queues sentinel ACL creds - password. Used when `spec.sentinelIsUsed: true` in `ApiManager CR` | `""` |
...
| REDIS_SENTINEL_USERNAME | System's redis sentinel ACL creds - username. Used when `spec.sentinelIsUsed: true` in `ApiManager CR` | `""` |
| REDIS_SENTINEL_PASSWORD | System's redis sentinel ACL creds - password. Used when `spec.sentinelIsUsed: true` in `ApiManager CR` | `""` |
